...
namespace SUMMS.Api.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> ProcessPayment([FromBody] PaymentRequest req)
        {
            ...
            string? last4 = null;
            if (!string.IsNullOrWhiteSpace(req.CardNumber) && req.CardNumber.Length >= 4)
            {
                last4 = req.CardNumber.Substring(req.CardNumber.Length - 4);
            }

            string? expMonth = null;
            string? expYear = null;
            if (!string.IsNullOrWhiteSpace(req.Expiry) && req.Expiry.Contains('/'))
            {
                var parts = req.Expiry.Split('/');
                if (parts.Length == 2)
                {
